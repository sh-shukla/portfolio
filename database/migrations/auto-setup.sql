-- Run this in Supabase SQL Editor to create tables and insert data
...
CREATE POLICY "Allow public insert to page_views" ON page_views FOR INSERT WITH CHECK (true);
...
-- Insert page views (using the visitor IDs from above)
INSERT INTO page_views (visitor_id, page, timestamp, time_spent)
SELECT 
  v.id,
  '/',
  v.timestamp,
  30000 + (ROW_NUMBER() OVER (ORDER BY v.created_at) - 1) * 10000
FROM visitors v;
...
SELECT 
  v.id,
  CASE 
    WHEN v.session_id = 'session_2' THEN '/about'
    WHEN v.session_id = 'session_3' THEN '/projects'
  END,
  v.timestamp + 30000,
  45000
FROM visitors v
WHERE v.session_id IN ('session_2', 'session_3');